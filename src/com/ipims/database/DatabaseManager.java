...
public class DatabaseManager {
				...
				if(affectedRows != 0)
				{
					...
					if(rs.next())
					{
						...
						if (user.getUsertype() == UserType.DOCTOR) {
							...
							newDoctorCategory(doc, doc.getCategory());
						}
					}
				}
				insertPatient.close();
	...
	/**
	 * Adds new doctor category for the doctor.
	 * @param doc
	 * @param category
	 */
	...
	public List<Appointment> getAppointmentsForDoctor(int doctorId, String date)
	{
		List<Appointment> appointmentList = new ArrayList<>();
	...
	private User createUser(ResultSet rs) throws SQLException {
		...
		if (user.getUsertype() == UserType.DOCTOR) {
			Doctor doc = (Doctor)user;
			doc.setCategory(getCategoryForDoctor(doc));
		}
		return user;
	}
